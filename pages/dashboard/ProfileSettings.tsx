...
import { resetUserPassword } from '../../services/mockData';
...
const ProfileSettings: React.FC = () => {
  ...
  const handleImageUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      // 2MB limit check to prevent localStorage crashes
  ...
  const handleSave = (e: React.FormEvent) => {
    ...
    // Wrapped in try-catch to prevent "Blank Screen" on storage failure
    try {
      setTimeout(() => {
        // 1. Update Password in "DB" if provided
        if (passwords.new) {
          resetUserPassword(initialUser.id, passwords.new);
        }

        // 2. Update Basic Info in session and "DB"
        // Ensure id and role are explicitly preserved from the current session
        ...
        // We no longer call window.location.reload() immediately 
        // to avoid race conditions with React rendering cycles
      }, 1000);
    } catch (err) {
      console.error("Profile Save Failed:", err);
      setErrorMsg("Failed to save changes. Your local storage might be full.");
      setIsSaving(false);
    }
  };
      ...
      <div className="flex flex-col items-center justify-center py-20 animate-in zoom-in-95 duration-500">
         ...
         <div className="flex gap-4">
            <button 
               ...
               className="px-10 py-4 bg-slate-900 text-white font-black text-[10px] uppercase tracking-widest rounded-2xl hover:bg-black transition-all"
            >
               Return to Profile
            </button>
            <Link to="/dashboard" className="px-10 py-4 bg-slate-100 text-slate-500 font-black text-[10px] uppercase tracking-widest rounded-2xl hover:bg-slate-200 transition-all">
               Go to Dashboard
            </Link>
         </div>
      </div>
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-12 animate-in slide-in-from-bottom-4 duration-500">
      ...
      {errorMsg && (
        <div className="p-6 bg-red-50 border border-red-100 rounded-3xl text-red-600 font-bold text-sm animate-in shake-in">
      ...
      <form onSubmit={handleSave} className="space-y-10">
          ...
          <div className="flex flex-col gap-16">
            ...
            <div className="flex flex-col md:flex-row gap-16 items-center">
              <div className="relative group">
                ...
                <button 
                  type="button"
                  onClick={() => fileInputRef.current?.click()}
                  className="absolute -bottom-4 left-1/2 -translate-x-1/2 px-4 py-2 bg-slate-900 text-white rounded-xl text-[9px] font-black uppercase tracking-widest shadow-xl whitespace-nowrap"
                >
                  Change Photo
                </button>
              </div>
            ...
            <div className="grid md:grid-cols-2 gap-8 border-t border-slate-100 pt-12">
               <div>
                  ...
                  <div className="relative">
                    ...
                    <input 
                      type="tel"
                      className="w-full bg-slate-50 border border-slate-100 rounded-2xl pl-14 pr-6 py-5 outline-none focus:ring-2 focus:ring-orange-500 font-bold text-slate-900"
               ...
               <div className="md:col-span-2">
                  ...
                  <textarea 
                    rows={4}
                    className="w-full bg-slate-50 border border-slate-100 rounded-2xl px-6 py-5 outline-none focus:ring-2 focus:ring-orange-500 font-medium text-slate-900 leading-relaxed"
                    placeholder="Describe your current profession or affiliation with the organisation..."
            ...
            <div className="border-t border-slate-100 pt-12">
               <h4 className="text-xl font-black uppercase tracking-tighter mb-8 flex items-center gap-3">
                  <i className="fas fa-shield-alt text-orange-500"></i> Security & Credentials
               </h4>
        ...
        <div className="bg-white p-12 rounded-[3rem] border border-slate-200 shadow-sm flex flex-col md:flex-row justify-between items-center gap-8">
          ...
          <div className="flex gap-4">
             <button 
                type="button"
                onClick={() => alert("Contact Super Admin to reset account metadata.")}
                className="px-8 py-3 bg-slate-100 text-slate-600 font-black text-[10px] uppercase tracking-widest rounded-xl hover:bg-slate-200 transition-all"
             >
                Reset Meta
             </button>
             <button 
                type="button"
                className="px-8 py-3 bg-slate-900 text-white font-black text-[10px] uppercase tracking-widest rounded-xl hover:bg-black transition-all shadow-lg"
             >
                Two-Factor Security
             </button>
          </div>
        ...
        <button 
          type="submit"
          disabled={isSaving}
          className="w-full py-6 bg-orange-600 text-white font-black rounded-[2.5rem] hover:bg-orange-700 transition-all shadow-2xl shadow-orange-100 flex items-center justify-center gap-3 uppercase text-xs tracking-widest disabled:opacity-50"
        >
