...
export const DashboardLayout: React.FC<DashboardLayoutProps> = ({
  ...
  const menuItems = [
    ...
    },
    {
      ...
      icon: "fa-heart-pulse",
      roles: [UserRole.SUPER_ADMIN, UserRole.MID_ADMIN, UserRole.STAFF_ADMIN],
    },
    {
      name: "News CMS",
      path: "/dashboard/news",
    ...
    },
    {
      ...
      roles: [UserRole.SUPER_ADMIN, UserRole.MID_ADMIN],
    },
    {
      name: "About",
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex font-inter overflow-x-hidden">
      <PageMeta
        title={`Dashboard | ${filteredMenu.find((m) => m.path === location.pathname)?.name || "Portal"}`}
        description="Naanghirisa admin dashboard for managing content, campaigns, donations, and user access."
      />

      {/* Sidebar - Desktop and Mobile Drawer */}
      <aside
        className={`fixed inset-y-0 left-0 w-64 bg-white text-slate-600 transition-transform duration-300 z-50 flex flex-col border-r border-slate-200 shadow-lg ${
      ...
      >
          ...
          {filteredMenu.map((item) => {
            ...
            return (
              <Link
                ...
                className={`flex items-center gap-1 rounded-sm px-3 py-1 transition-all group ${
                  isActive
                    ? "bg-red-950 text-white shadow-lg"
        ...
        <div className="p-4 border-t border-white/5">
          <button
            ...
            className="w-full py-3 rounded-lg bg-red-500/10 text-red-400 text-[10px] font-black uppercase tracking-widest hover:text-white hover:bg-red-500 transition-all"
          >
      ...
      {/* Main Content Area */}
      <div className="flex-grow transition-all duration-300 flex flex-col min-w-0 lg:ml-64">
        <header className="fixed top-0 left-0 lg:left-64 right-0 h-16 bg-white/95 backdrop-blur-xl border-b border-slate-200 z-30 px-4 md:px-6 flex justify-between items-center shadow-sm">
          <div className="flex items-center gap-4">
            ...
            <div className="hidden sm:block">
              <h2 className="text-xl font-black uppercase tracking-tight truncate">
                {filteredMenu.find((m) => m.path === location.pathname)?.name ||
                  "Dashboard"}
              </h2>
            </div>
          </div>
          ...
          <div className="flex items-center gap-4 md:gap-6">
            <div className="relative" ref={notifRef}>
              <button
                ...
                className="w-10 h-10 bg-slate-50 rounded-xl flex items-center justify-center text-slate-600 relative border border-slate-200 hover:bg-slate-100 transition-colors"
              >
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      {unreadCount > 0 && (
                        <button
                          ...
                          className="text-[9px] font-black uppercase tracking-widest text-slate-900 hover:text-orange-600"
                        >
                          Mark all read
                        </button>
                      )}
                    ...
                    {notifications.length > 0 ? (
                      notifications.map((n) => (
                        <button
                          ...
                          className={`w-full text-left p-5 border-b border-slate-50 cursor-pointer transition-colors hover:bg-slate-50 ${
                          ...
                          <div className="flex items-start gap-3">
                            <span
                              className={`mt-1 h-2.5 w-2.5 rounded-full ${
                                n.read ? "bg-slate-300" : "bg-orange-500"
                              }`}
                            ></span>
            ...
            <div className="flex items-center gap-3 pl-4 border-l border-slate-200">
              <div className="text-right hidden sm:block">
                ...
                <p className="text-[8px] text-orange-600 font-black">
                  {user.role.replace("_", " ")}
                </p>
              </div>
        ...
        <div className="pt-16 flex-grow flex flex-col min-w-0">
