...
export const DashboardLayout: React.FC<DashboardLayoutProps> = ({
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex font-inter overflow-x-hidden">
      ...
      <aside
        className={`fixed inset-y-0 left-0 w-64 bg-slate-50 text-slate-500 transition-transform duration-300 z-50 flex flex-col border-r border-slate-300 shadow-lg ${isSidebarOpen ? "translate-x-0" : "-translate-x-full lg:translate-x-0"}`}
      >
        ...
        <nav className="flex-grow px-3 py-2 space-y-1 overflow-y-auto custom-scrollbar">
          {filteredMenu.map((item) => {
            ...
            return (
              ...
              >
                <i
                  className={`fas ${item.icon} w-5 text-sm ${isActive ? "text-white" : "text-slate-500 group-hover:text-orange-400"}`}
                ></i>
      ...
      {/* Main Content Area */}
      <div
        className={`flex-grow transition-all duration-300 flex flex-col min-w-0 ${isSidebarOpen ? "lg:ml-64" : "lg:ml-64"}`}
      >
        <header className="h-20 bg-white/80 backdrop-blur-xl border-b border-orange-400/50 sticky fixed top-0 z-30 px-6 md:px-10 flex justify-between items-center">
          <div className="flex items-center gap-4">
            ...
            <h2 className="hidden sm:block text-xl font-black uppercase tracking-tighter truncate">
          ...
          <div className="flex items-center gap-4 md:gap-6">
            <div className="relative" ref={notifRef}>
              <button
                ...
                className="w-10 h-10 bg-slate-100 rounded-xl flex items-center justify-center text-slate-500 relative"
              >
                    ...
                    {notifications.length > 0 ? (
                      notifications.map((n) => (
                        <div
                          ...
                          className={`p-5 border-b border-slate-50 cursor-pointer hover:bg-slate-50 ${!n.read ? "bg-orange-50/20" : ""}`}
                        >
        ...
        <div className="p-4 md:p-8 flex-grow">{children}</div>
