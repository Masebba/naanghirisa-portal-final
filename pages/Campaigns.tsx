...
const Campaigns: React.FC = () => {
  ...
  const totalRaised = campaigns.reduce((sum, item) => sum + item.amountRaised, 0);
  const activeCampaigns = campaigns.filter(item => item.status === 'Active').length;
  const closedCampaigns = campaigns.filter(item => item.status === 'Closed').length;
  const totalTargets = campaigns.reduce((sum, item) => sum + item.targetAmount, 0);
  const overallProgress = totalTargets > 0 ? Math.min(100, Math.round((totalRaised / totalTargets) * 100)) : 0;
  ...
  return (
    <div className="bg-white min-h-screen">
      <div className="py-24 text-center text-white" style={{ backgroundColor: COLORS.primary, backgroundImage: pageContent.campaignsHeroImage ? `linear-gradient(rgba(88,0,0,0.82), rgba(88,0,0,0.82)), url(${pageContent.campaignsHeroImage})` : undefined, backgroundSize: 'cover', backgroundPosition: 'center' }}>
        <div className="max-w-7xl mx-auto px-4">
          ...
          <p className="text-xl opacity-90 max-w-2xl mx-auto font-medium">
            Your contributions directly fuel our community initiatives. Every gift strengthens a child, a family, and a future.
          </p>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 py-20">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-20 text-center">
          <div className="p-8 rounded-[2rem] bg-slate-50 border border-slate-100">
            <p className="text-3xl font-black text-orange-500 mb-1">${totalRaised.toLocaleString()}</p>
            <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">Total Raised</p>
          </div>
          <div className="p-8 rounded-[2rem] bg-slate-50 border border-slate-100">
            <p className="text-3xl font-black text-orange-500 mb-1">{activeCampaigns}</p>
            <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">Active Campaigns</p>
          </div>
          <div className="p-8 rounded-[2rem] bg-slate-50 border border-slate-100">
            <p className="text-3xl font-black text-orange-500 mb-1">{overallProgress}%</p>
            <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">Portfolio Progress</p>
          </div>
          <div className="p-8 rounded-[2rem] bg-slate-50 border border-slate-100">
            <p className="text-3xl font-black text-orange-500 mb-1">{closedCampaigns}</p>
        ...
        {hasCampaigns ? (
          <>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10 mb-20">
              {displayedCampaigns.map((camp) => {
                const progress = camp.targetAmount > 0 ? (camp.amountRaised / camp.targetAmount) * 100 : 0;
          ...
          <div className="mb-20 rounded-[3rem] border border-dashed border-slate-200 bg-slate-50 px-8 py-16 text-center">
            <h2 className="text-3xl font-black mb-4" style={{ color: COLORS.primary }}>No campaigns available yet</h2>
            <p className="max-w-2xl mx-auto text-slate-500 leading-relaxed">
              Campaigns will appear here once the team creates them in the dashboard. Until then, donations can still be received through the main donation page and assigned to future initiatives.
            </p>
            <div className="mt-8 flex justify-center gap-4 flex-wrap">
              <Link to="/donate" className="px-8 py-4 rounded-2xl text-white font-black uppercase tracking-widest text-[10px] shadow-xl" style={{ backgroundColor: COLORS.secondary }}>
                Donate Now
              </Link>
              <Link to="/login" className="px-8 py-4 rounded-2xl border border-slate-200 bg-white text-slate-700 font-black uppercase tracking-widest text-[10px]">
                Admin Setup
              </Link>
            </div>
          </div>
        ...
        <div className="grid lg:grid-cols-2 gap-16 items-center py-20 border-t border-slate-100">
          <div>
            ...
            <p className="text-slate-600 text-lg leading-relaxed mb-10">
              We track donations, campaign progress, and spending with a transparency-first workflow so supporters can follow real outcomes.
            </p>
            <div className="space-y-6">
              <div className="flex gap-6">
                ...
                <div>
                  ...
                  <p className="text-slate-500 text-sm">Donations are captured through the portal and tracked in the ledger for review.</p>
                </div>
              </div>
              <div className="flex gap-6">
                ...
                <div>
                  ...
                  <p className="text-slate-500 text-sm">Funds flow into defined campaigns and program allocations based on organisational priorities.</p>
                </div>
              </div>
              <div className="flex gap-6">
                ...
                <div>
                  ...
                  <p className="text-slate-500 text-sm">Recent donations update campaign progress immediately inside the portal.</p>
                </div>
              </div>
            </div>
          </div>
            ...
            {recentSupport.length > 0 ? (
              <div className="space-y-4">
                {recentSupport.map((donation, idx) => (
                  <div key={donation.id} className="flex justify-between items-center p-4 bg-white/5 rounded-2xl border border-white/10 hover:bg-white/10 transition-colors">
                    <div className="flex items-center gap-3">
                      ...
                      <span className="text-sm font-bold">{donation.donorName || 'Supporter'}</span>
                    </div>
                    <div className="text-right">
                      <p className="text-orange-400 font-black text-sm">${donation.amount.toLocaleString()}</p>
                      <p className="text-[9px] text-slate-400 uppercase font-bold tracking-widest">
                        {donation.category || donation.campaignId || `Gift #${idx + 1}`}
                      </p>
                    </div>
                  </div>
                ))}
              ...
              <div className="rounded-3xl border border-white/10 bg-white/5 p-8 text-center">
                <p className="text-slate-300 text-sm leading-relaxed">
                  Recent donations will appear here once supporters start contributing through the portal.
                </p>
              </div>
            )}
        ...
        <div className="mt-24 text-center max-w-4xl mx-auto">
          ...
          <div className="grid gap-4 text-left">
            {[
              ...
              { q: 'Can I donate to a specific child?', a: 'While general campaigns support cohorts, you can use our Child Sponsorship Program to support an individual student.' },
              { q: 'What happens if a campaign exceeds its goal?', a: 'Surplus funds are allocated to the next most urgent community need within the same category.' },
            ].map((faq, idx) => (
