        ...
        {hasPrograms ? (
          <div className="grid grid-cols-1 gap-8 md:grid-cols-2 lg:grid-cols-3 mb-16">
            {displayedPrograms.map((program) => (
              <div key={program.id} className="group flex flex-col overflow-hidden rounded-[2.5rem] border border-slate-100 bg-white shadow-sm transition-all duration-500 hover:shadow-2xl">
                <div className="relative h-64 overflow-hidden">
                  <img src={program.image} alt={program.name} className="h-full w-full object-cover transition-transform duration-700 group-hover:scale-110" />
                  <div className="absolute left-4 top-4 flex gap-2">
                    <span className={`inline-block rounded-full px-3 py-1 text-[10px] font-black uppercase tracking-wider shadow-lg ${program.status === 'Running' ? 'bg-emerald-500 text-white' : 'bg-amber-500 text-white'}`}>
                      {program.status}
                    </span>
                ...
                <div className="flex flex-grow flex-col p-8">
                  <h3 className="mb-3 text-xl font-black line-clamp-1" style={{ color: COLORS.primary }}>{program.name}</h3>
                  <p className="mb-8 flex-grow text-sm leading-relaxed text-slate-600 line-clamp-3">{program.description}</p>
                  <div className="mt-auto flex flex-col gap-4">
                    <div className="flex items-center justify-between border-t border-slate-100 pt-4">
                      <div>
                        <p className="text-[9px] font-black uppercase tracking-widest text-slate-400">Budget</p>
                        <p className="text-base font-black" style={{ color: COLORS.secondary }}>${program.allocatedBudget.toLocaleString()}</p>
                      </div>
                      <Link to={`/programs/${program.id}`} className="rounded-xl bg-slate-100 px-5 py-2 text-[10px] font-black uppercase tracking-widest text-slate-900 transition hover:bg-slate-200">Read more</Link>
                    </div>
                    <button onClick={() => handleDonateClick(program.id)} className="w-full rounded-xl bg-orange-600 py-3 text-[10px] font-black uppercase tracking-widest text-white shadow-md transition hover:bg-orange-700">Donate to this program</button>
                  </div>
                </div>
              </div>
            ))}
          ...
          <div className="mb-16 rounded-[3rem] border border-dashed border-slate-200 bg-slate-50 px-8 py-16 text-center">
            ...
            <h3 className="text-2xl font-black text-slate-900">No programmes published yet</h3>
